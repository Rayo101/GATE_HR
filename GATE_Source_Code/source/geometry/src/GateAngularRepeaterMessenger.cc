  ...
  :GateObjectRepeaterMessenger(itsAngularRepeater)
{
  ...
  cmdName = GetDirectoryName()+"excludeBlock1";
  ExcludeBlock1Cmd = new G4UIcmdWithAnInteger(cmdName,this);
  ExcludeBlock1Cmd->SetGuidance("Select the first block to remove");
  ExcludeBlock1Cmd->SetParameterName("block1",false);

  cmdName = GetDirectoryName()+"excludeBlock2";
  ExcludeBlock2Cmd = new G4UIcmdWithAnInteger(cmdName,this);
  ExcludeBlock2Cmd->SetGuidance("Select the second block to remove");
  ExcludeBlock2Cmd->SetParameterName("block2",false);

  cmdName = GetDirectoryName()+"excludeBlock3";
  ExcludeBlock3Cmd = new G4UIcmdWithAnInteger(cmdName,this);
  ExcludeBlock3Cmd->SetGuidance("Select the third block to remove");
  ExcludeBlock3Cmd->SetParameterName("block3",false);

  cmdName = GetDirectoryName()+"excludeBlock4";
  ExcludeBlock4Cmd = new G4UIcmdWithAnInteger(cmdName,this);
  ExcludeBlock4Cmd->SetGuidance("Select the forth block to remove");
  ExcludeBlock4Cmd->SetParameterName("block4",false);
...
GateAngularRepeaterMessenger::~GateAngularRepeaterMessenger()
{
  ...
  delete Shift7Cmd;
  delete Shift8Cmd;
  delete ExcludeBlock1Cmd;
  delete ExcludeBlock2Cmd;
  delete ExcludeBlock3Cmd;
  delete ExcludeBlock4Cmd;
...
void GateAngularRepeaterMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  else if ( command==ExcludeBlock1Cmd)
    { GetAngularRepeater()->SetExcludeBlock1(ExcludeBlock1Cmd->GetNewIntValue(newValue)) ;}
  else if ( command==ExcludeBlock2Cmd)
    { GetAngularRepeater()->SetExcludeBlock2(ExcludeBlock2Cmd->GetNewIntValue(newValue)) ;}
  else if ( command==ExcludeBlock3Cmd)
    { GetAngularRepeater()->SetExcludeBlock3(ExcludeBlock3Cmd->GetNewIntValue(newValue)) ;}
  else if ( command==ExcludeBlock4Cmd)
    { GetAngularRepeater()->SetExcludeBlock4(ExcludeBlock4Cmd->GetNewIntValue(newValue)) ;}
  else
